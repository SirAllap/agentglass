 ...
 *
 *  Exported because Home draws the same boundary and must draw it in the same
 *  place: a phone that says an agent is thinking on one screen and stopped on
 *  the next has two opinions about one agent. */
export const QUIET_MS = 4 * 60_000;
/** Past this, it is not "just stopped" any more, it is yesterday's business
 *  and it does not belong in a queue you are meant to be able to empty — nor
 *  in a list of what is running. */
