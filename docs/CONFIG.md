...
| `AGENTGLASS_DB` | `~/.local/share/agentglass/agentglass.db` | SQLite file path. The default lives under `$XDG_DATA_HOME` (or `~/.local/share`), created `0700`. A pre-existing `agentglass.db` **in the working directory** wins. Only an installed app opens the default: a run from source (`bun run dev`, `bun -e`, a headless `bun run src/index.ts`) or a binary built inside a checkout (`electron/staging`, `electron/dist`) refuses to start until it is told where its database is — this variable, or `AGENTGLASS_STATE_DIR`. Naming the default path here opens it on purpose. A database written by a newer release that an older one cannot read is refused with "the database is newer than this app" instead of an SQLite error. A second server on the same file runs with its transcript scanner disabled (see the claim above), so it will not corrupt anything, but it also will not scan. |
