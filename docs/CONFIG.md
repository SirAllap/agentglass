...
| `AGENTGLASS_DB` | `~/.local/share/agentglass/agentglass.db` | SQLite file path. The default lives under `$XDG_DATA_HOME` (or `~/.local/share`), created `0700`. An `agentglass.db` **in the working directory** is never opened. When the data dir has no database yet it is copied there once (with its `-wal`; the original is left untouched and the copy is refused if it does not pass `quick_check`); otherwise the app shows a notice with both paths and the `mv` that would switch them, and the server logs the same once at startup. So `bun run dev` in any checkout lands on the shared history under `$XDG_DATA_HOME`, the same file the installed app uses. Isolating `XDG_DATA_HOME` alone is not enough either if you also want the *installed* app's database left alone — set this variable. A second server on the same file runs with its transcript scanner disabled (see the claim above), so it will not corrupt anything, but it also will not scan. |
